...
- **No em dashes.** Never use `—` (U+2014 em dash) in any text in this project: not in source comments, not in user-visible UI strings, not in docs, not in commit messages, not in PR descriptions, not in commit message bodies. They read as LLM-generated and the user has explicitly asked them removed. Use periods, commas, colons, parens, or regular hyphens as the context calls for.
